...
def _build_proposals(
    *,
    ...
    candidates: list[str] = []
    if issue_type == "empty_text":
        if original_text.strip():
            candidates.append(original_text.strip())
        candidates.append("Completar texto con una version clara y apta para edad objetivo.")
    elif issue_type == "too_short":
        if original_text.strip() and original_text.strip() != current_text.strip():
            candidates.append(original_text.strip())
        candidates.append(f"{current_text.strip()} Añadir una frase breve que complete la idea principal.")
    elif issue_type == "placeholder_marker":
        cleaned = re.sub(r"\b(TODO|FIXME)\b", "", current_text, flags=re.IGNORECASE).strip()
        candidates.append(cleaned or original_text.strip() or "Texto revisado sin marcadores pendientes.")
    elif issue_type == "draft_marker":
        cleaned = current_text.replace("[[", "").replace("]]", "").strip()
        candidates.append(cleaned or original_text.strip() or "Texto revisado sin marcas de borrador.")
    else:
        candidates.append(current_text.strip() or original_text.strip())
...
def _build_finding(
    *,
    ...
    finding_id = build_finding_id(
        ...
        content=current_text or original_text,
    )
    return {
        ...
        "proposals": _build_proposals(issue_type=issue_type, current_text=current_text, original_text=original_text),
    }


def detect_text_findings(story: dict[str, Any], severity: str) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    for page in story.get("pages", []):
        page_number = int(page.get("page_number", 0))
        text_obj = page.get("text", {})
        current_text = str(text_obj.get("current", "") or "")
        original_text = str(text_obj.get("original", "") or "")
        ...
        if severity == "critical":
            ...
            if normalized and _word_count(normalized) < 12:
                findings.append(
                    _build_finding(
                        severity=severity,
                        page_number=page_number,
                        issue_type="too_short",
                        message="El texto es demasiado corto para lectura fluida.",
                        current_text=current_text,
                        original_text=original_text,
                    )
                )
            ...
            if re.search(r"\b(TODO|FIXME)\b", normalized, flags=re.IGNORECASE):
                findings.append(
                    _build_finding(
                        ...
                        message="El texto contiene marcadores pendientes (TODO/FIXME).",
                        current_text=current_text,
                        original_text=original_text,
                    )
                )
            ...
            if "[[" in normalized and "]]" in normalized:
                findings.append(
                    _build_finding(
                        ...
                        message="El texto contiene marcas de borrador [[...]].",
                        current_text=current_text,
                        original_text=original_text,
                    )
                )
...
def apply_decisions(
    *,
    ...
    for decision in decisions:
        ...
        if not finding or finding.get("stage") != "text":
            raise ValueError(f"finding_id no valido para texto: {finding_id}")
        ...
        if custom_text is None:
            custom_text = decision.get("final_text")
        final_text = resolve_option_value(
            finding,
            selected_option=selected_option,
            custom_value=str(custom_text) if custom_text is not None else None,
        )

        page.setdefault("text", {})["current"] = final_text
...
def run(story_rel_path: str, severity: str, target_age: int | None, apply_decision_json: str | None) -> dict[str, Any]:
    ...
    findings = detect_text_findings(story, severity)
    upsert_open_findings(review, stage="text", severity=severity, findings=findings)
    review.setdefault("metrics", {}).setdefault("runs", {}).setdefault("texto", 0)
    review["metrics"]["runs"]["texto"] = int(review["metrics"]["runs"]["texto"]) + 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Detectar y aplicar decisiones de adaptacion textual por severidad.")
