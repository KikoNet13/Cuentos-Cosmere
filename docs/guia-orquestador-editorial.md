...
Documento reducido. La operación oficial y vigente del flujo 3 IAs vive en `AGENTS.md`.
...
5. ChatGPT Project genera imágenes a partir de prompts + anchors (`reference_ids` basados en `meta.anchors[].image_filenames`).
6. Operación recomendada en UI para imagen:
   - abrir `/_flow/image`,
   - la app muestra solo el primer pendiente global,
   - copiar prompt + refs, generar en ChatGPT Project,
   - usar "Pegar y guardar";
   - recarga automática al siguiente pendiente.
